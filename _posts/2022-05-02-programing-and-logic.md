...
(문제는 SW Expert Academy 학습자료에서 확인하세요)

👉 Tip. 가정인 P(x)가 거짓이거나, Q(x)가 참이면 전체 명제는 무조건 참이 된다.
...
여기서는 P(x)가 '실수 x라면'이기 때문에 거짓일 수가 없다.

그렇다면 Q(x)가 참이라는 것을 증명해보자.

1. 결과 명제 : '실수 x < -1이면, x² + 1/4 > 0이다.'
2. x² + 1/4 > 0은 x² > - 1/4로 바꿀 수 있는데, 실수 x는 -1보다 작으므로 무조건 음수이다.
3. x²은 음수와 음수의 곱이므로 무조건 양수가 되고, 무조건 - 1/4보다 클 수밖에 없다.
4. 즉, Q(x)가 항상 참이되고, 자연스레 전체 명제도 무조건 참이 된다.

### **↘︎ 문제 1-2**

마찬가지로 가정은 무조건 참이므로, 결과가 항상 참이 되는지 확인하면 된다.

1. 결과 명제 : '4n³ + 6n² + 12가 짝수이다.'
2. 4n³ + 6n² + 12 = 2(2n³ + 3n² + 6)
3. 어떤 수에 2를 곱한 값은 무조건 짝수이므로 Q(x)는 항상 참이다.
4. 결과 명제가 무조건 참이므로 전체 명제 역시 항상 참이다.
...
전체 명제의 가정 명제인 P'(x)는 항상 참이지만, 결과 명제의 가정(P(x))이 거짓임을 중명할 수 있다.

1. 2x² -4x + 4 < 0 이라면, x > 8 이다.
2. 2(x² -2x + 2) < 0으로 바꾸었을 때에 2와 어떤 실수의 값은 무조건 양수이므로 0보다 작을 수 없다.
3. P(x)가 거짓이므로 결과 명제인 Q(x)는 항상 참이 된다.
4. 즉, 전체 명제의 결과가 항상 참이므로 전체 명제도 항상 참이다.

### **↘︎ 문제 2-2**

이 문제는 전체 명제의 가정인 P(x)가 거짓임을 판별하여 풀 수 있다.

1. 4n³ + 6n² + 11이 짝수이면...
2. 2(2n³ + 3n² + 5) + 1로 바꾸어 표현 가능하다.
3. 2와 실수의 곱은 항상 짝수인데, 짝수 + 1은 항상 홀수가 된다.
4. 즉, 가정이 거짓이므로 본 명제는 항상 참이 된다.
